...
export class mHelper {
    ...
    getUser(username,cb){
        this.db_user.getUser(username, function (err, response) {
            if (err) {
              ...
              throw new Error(' could not get this user');
            } else {
              cb(response);
            }
          });
    ...
    updateUserInfo(username,dbconfig:Idbconfig){
        this.db_user.putUser(username, {
            metadata :{dbconfig:dbconfig},
    ...
    assigntpermissiondb() {
        let dbname, username, role, isadmin ;
